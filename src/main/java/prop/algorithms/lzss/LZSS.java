...
public class LZSS implements BaseAlgorithm {
  ...
  public byte[] encode(byte[] input) {
    ...
    try {
      WindowBuffer w = new WindowBuffer((short) BUFFER_SIZE_SEARCH, (short) BUFFER_SIZE_LOOKAHEAD,
          inputSB);
  ...
  public byte[] decode(byte[] input) {
    ...
    byte[] flagArray;
      if (numFlags % 8 != 0) { flagArray = new byte[(numFlags / 8) + 1]; } else {
          flagArray = new byte[numFlags / 8];
      }
